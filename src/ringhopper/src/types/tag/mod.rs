...
///   - The HEK is inconsistent with how it handles these paths, so it is better to remove paths that begin with path
...
///     directories. Ringhopper supports using multiple tags directories simultaneously, so that feature should be used,
///     instead.
///
