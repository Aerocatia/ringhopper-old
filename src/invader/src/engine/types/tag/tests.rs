use crate::{TagBlockFn, FieldReference, BlockArray, Vector3D, TagField};
...
impl TagBlockFn for MyTagBlock {
    ...
    fn field_at_index(&self, index: usize) -> TagField {
        if index == 0 {
            return TagField::Value(FieldReference { field: &self.some_field, name: "some field", comment: "" })
        }
        else if index == 1 {
            return TagField::Array(&self.another_field)
        }
        else if index == 2 {
            return TagField::Value(FieldReference { field: &self.useless_field, name: "useless field", comment: "" })
        }
    ...
    fn field_at_index_mut(&mut self, index: usize) -> TagField {
        if index == 0 {
            return TagField::MutableValue(FieldReference { field: &mut self.some_field, name: "some field", comment: "" })
        }
        else if index == 1 {
            return TagField::MutableArray(&mut self.another_field)
        }
        else if index == 2 {
            return TagField::MutableValue(FieldReference { field: &mut self.useless_field, name: "useless field", comment: "" })
        }
...
fn test_access() {
    ...
    let mut vector_level_0_0 = match block.field_at_index_mut(0) {
        TagField::MutableValue(v) => v.field.downcast_mut::<Vector3D>().unwrap(),
        _ => panic!()
    };
    ...
    // Access the array
    let array_level_0 = match block.field_at_index_mut(1) {
        TagField::MutableArray(v) => v,
        _ => panic!()
    };
    assert_eq!(2, array_level_0.len());

    let block_level_1_0 = array_level_0.block_at_index_mut(0);
    let mut vector_level_1_0 = match block_level_1_0.field_at_index_mut(0) {
        TagField::MutableValue(n) => n.field.downcast_mut::<Vector3D>().unwrap(),
        _ => panic!()
    };
    ...
    let block_level_1_0_1 = match block_level_1_0.field_at_index(1) {
        TagField::Array(v) => v,
        _ => panic!()
    };
    ...
    let mut vector_level_1_1 = match block_level_1_1.field_at_index_mut(0) {
        TagField::MutableValue(n) => n.field.downcast_mut::<Vector3D>().unwrap(),
        _ => panic!()
    };
    ...
    let array_level_1 = match block_level_1_1.field_at_index_mut(1) {
        TagField::MutableArray(n) => n,
        _ => panic!()
    };
    let block_level_2_0 = array_level_1.block_at_index_mut(0);
    let mut vector_level_2_0 = match block_level_2_0.field_at_index_mut(0) {
        TagField::MutableValue(n) => n.field.downcast_mut::<Vector3D>().unwrap(),
        _ => panic!()
    };
    ...
    let array_level_2_1 = match block_level_2_0.field_at_index(1) {
        TagField::Array(n) => n,
        _ => panic!()
    };
    ...
    fn recursion(block: &mut dyn TagBlockFn) -> u32 {
        ...
        for i in 0..block.field_count() {
            match block.field_at_index_mut(i) {
                TagField::MutableArray(mut array) => {
                    for i in 0..array.len() {
                        count += recursion(&mut array[i])
                    }
                },
                TagField::MutableValue(value) => {
                    if let Some(vector) = value.field.downcast_mut::<Vector3D>() {
                        vector.z /= 2.0;
                    }
                    else if let Some(value) = value.field.downcast_mut::<u32>() {
                        count += *value;
                    }
                }
                _ => unreachable!()
