...
pub fn recover_jms(model: Model, tag_file: &TagFile, data_dir: &Path, overwrite: bool) -> ErrorMessageResult<RecoverProcessedResult> {
    ...
    let mut result = RecoverProcessedResult::DataAlreadyExists;
    ...
    for (p, jms_arr) in all_jms {
        for i in 0..jms_arr.len() {
            ...
            result = RecoverProcessedResult::Recovered;
        }
    }
...
pub fn recover_processed_models(tag_data: &[u8], tag_file: &TagFile, data_dir: &Path, overwrite: bool) -> ErrorMessageResult<RecoverProcessedResult> {
    let model = *Model::from_tag_file(tag_data)?.data;
    recover_jms(model, tag_file, data_dir, overwrite)
}

pub fn recover_processed_gbxmodels(tag_data: &[u8], tag_file: &TagFile, data_dir: &Path, overwrite: bool) -> ErrorMessageResult<RecoverProcessedResult> {
